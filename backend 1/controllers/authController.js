...
// Get current admin profile (isolated - each admin sees only their own data)
const getProfile = async (req, res) => {
  try {
    // req.admin is already loaded by verifyToken middleware with all fields except password
    // This ensures complete isolation - each admin only sees their own profile including signature_photo
...
// Upload or update admin signature image (isolated - each admin can only upload their own signature)
const uploadSignature = async (req, res) => {
  try {
    ...
    // Isolation: req.admin.id ensures each admin can only upload/modify their own signature
    const adminId = req.admin.id;
    ...
    // Compute relative path from backend root
    // req.file.path is already absolute, we need relative from backend root
    const backendRoot = path.join(__dirname, '..');
    let relativePath = path.relative(backendRoot, req.file.path);
    
    // Normalize path separators to forward slashes for consistency (works on both Windows and Linux)
    ...
    // Delete previous signature file if it exists (before saving new one)
    if (admin.signature_photo) {
      const deleted = deleteFile(admin.signature_photo);
      if (deleted) {
        console.log(`🗑️  Deleted previous signature: ${admin.signature_photo}`);
      }
    }

    // Update signature_photo field with normalized path
    admin.signature_photo = relativePath;
    
    try {
      await admin.save();
    } catch (saveError) {
      console.error('Error saving signature_photo to database:', saveError);
      // Check if it's a column not found error
      if (saveError.name === 'SequelizeDatabaseError' && saveError.message && saveError.message.includes('Unknown column')) {
        return res.status(500).json({
          success: false,
          error: 'Database column missing. Please run: ALTER TABLE admins ADD COLUMN signature_photo VARCHAR(512) NULL;'
        });
      }
      throw saveError;
    }
