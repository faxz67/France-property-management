...
const PaymentsManagement: React.FC = () => {
  ...
  const fetchBills = async (skipLoading = false) => {
    try {
      ...
      if (response.data?.success && response.data?.data?.bills) {
        ...
        console.log('📋 Raw bills from API:', rawBills.length);
        ...
        const processedBills = rawBills.map((bill: any) => {
          ...
          return {
            ...
            created_at_fr: new Date(bill.created_at).toLocaleDateString('fr-FR'),
            due_date_fr: dueDate ? dueDate.toLocaleDateString('fr-FR') : null,
            payment_date_fr: paymentDate ? paymentDate.toLocaleDateString('fr-FR') : null,
            // Format currency
            amount_formatted: new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'EUR' }).format(bill.amount || bill.total_amount || 0),
            rent_amount_formatted: bill.rent_amount ? new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'EUR' }).format(bill.rent_amount) : null,
            charges_formatted: bill.charges ? new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'EUR' }).format(bill.charges) : null,
            total_amount_formatted: new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'EUR' }).format(bill.total_amount || bill.amount || 0),
            // Add status analysis
            is_overdue: isOverdueBill,
            days_until_due: daysUntilDue || 0,
            status_fr: bill.status === 'PENDING' ? 'En attente' : 
                      bill.status === 'PAID' ? 'Payé' : 
                      bill.status === 'OVERDUE' ? 'En retard' : 
                      bill.status === 'RECEIPT_SENT' ? 'Reçu envoyé' : bill.status,
      ...
      console.log('🔄 Using mock bills due to API error...');
      
      // Use mock bills as fallback
      const mockBills = [
        {
          id: 1,
          tenant_id: 1,
      ...
      setBills(mockBills);
      console.log('📊 Mock bills loaded:', mockBills.length, 'bills');
      setError('');
      
      // Calculate stats from mock bills
      await fetchStats();
  ...
  // Get owner information from properties data
  const getOwnerInfo = () => {
    if (properties.length > 0) {
      // Use the first property as the owner's main property
      const mainProperty = properties[0];
      return {
        name: 'Hisham',
        company: 'Gestion Locative',
        address: mainProperty.address || '123 Rue de la Paix',
  ...
  const handleDownloadBill = async (billId: number) => {
    try {
      ...
      if (bill && bill.tenant && bill.tenant.name && bill.month) {
      ...
      {stats && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div className="bg-white p-6 rounded-lg shadow smooth-transition hover-lift fade-in">
            <div className="flex items-center">
              <DollarSign className="w-8 h-8 text-blue-600 smooth-transition" />
              <div className="ml-4">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {bills.map((bill, index) => (
                <tr key={bill.id} className="hover:bg-gray-50 smooth-transition hover-lift fade-in" style={{ animationDelay: `${index * 0.1}s` }}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {bill.payment_date ? new Date(bill.payment_date).toLocaleDateString('fr-FR') : '-'}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
