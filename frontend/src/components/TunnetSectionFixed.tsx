...
const TunnetSectionFixed: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let errorMsg = 'Échec de l\'ajout du locataire. Veuillez réessayer.';
      ...
      if (err?.response?.status === 400) {
        ...
        errorMsg = 'Accès refusé. Vous ne pouvez ajouter des locataires qu\'à vos propres propriétés.';
      } else if (err?.response?.status === 404) {
        errorMsg = 'Propriété non trouvée. Veuillez sélectionner une propriété valide.';
      ...
      if (errorMsg.includes('Too many requests') || err?.response?.status === 429) {
        errorMsg = 'Trop de requêtes. Veuillez attendre un moment avant de réessayer.';
      }
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-2xl md:text-3xl font-bold mb-6 md:mb-8 fade-in-soft text-center md:text-left">
          Tenant Management
        </h1>
      

      {/* Status Panels */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 mb-6 md:mb-8 items-stretch">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-3 md:gap-4 mb-5 md:mb-7">
      ...
      {/* Tenants Grid - 1 column on small screens, 2/3 on larger screens */}
      <div className="w-full grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 md:gap-6 items-stretch">
        {filteredTenants.map((tenant, index) => (
          <div 
            key={`tenant-${tenant.id}-${normalizeStatus(tenant.status)}-${index}`} 
            className="w-full bg-white rounded-2xl shadow-md border border-gray-100 overflow-hidden smooth-transition hover-lift fade-in min-h-[260px] sm:min-h-[280px] md:min-h-[320px] lg:min-h-[340px] flex flex-col"
            style={{ animationDelay: `${index * 0.1}s` }}
          >
            <div className="p-4 md:p-5 lg:p-6 flex-1 flex flex-col">
              {/* Header: avatar + infos */}
              <div className="flex items-center space-x-3 mb-3 md:mb-4">
                <div className="w-11 h-11 md:w-12 md:h-12 bg-blue-100 rounded-full flex items-center justify-center">
                  <span className="text-blue-600 font-semibold text-lg">
                    {tenant.fullName?.charAt(0) || 'T'}
                  </span>
                </div>
                <div>
                  <h3 className="font-semibold text-gray-900 text-sm md:text-base">{tenant.fullName}</h3>
                  <p className="text-xs md:text-sm text-gray-500 break-all">{tenant.email}</p>
                </div>
              </div>
              
              {/* Infos principales */}
              <div className="space-y-2 text-sm text-gray-600 mt-1 flex-1">
              ...
              <div className="mt-3 pt-3 border-t border-gray-100 flex justify-end space-x-1.5 md:space-x-2">
                <button
                  onClick={() => setDocumentsModalTenant(tenant)}
                  className="p-2 md:p-2.5 text-green-600 bg-green-50 hover:bg-green-100 rounded-lg nav-transition nav-hover-scale border border-green-100"
                ...
                <button
                  ...
                  className="p-2 md:p-2.5 text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-lg nav-transition nav-hover-scale border border-blue-100"
                ...
                <button
                  ...
                  className="p-2 md:p-2.5 text-red-600 bg-red-50 hover:bg-red-100 rounded-lg nav-transition nav-hover-scale hover:text-red-700 border border-red-100"
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-3 sm:p-4 md:p-6 overflow-y-auto">
          <div className="bg-white rounded-lg w-full max-w-2xl mx-auto my-auto shadow-xl">
            ...
            <div className="sticky top-0 bg-white z-10 px-4 sm:px-6 pt-4 sm:pt-6 pb-3 border-b border-gray-200 rounded-t-lg">
              <div className="flex justify-between items-center">
                <h2 className="text-lg sm:text-xl md:text-2xl font-semibold text-gray-800">Ajouter un nouveau locataire</h2>
                <button
            ...
            <div className="px-4 sm:px-6 py-4 sm:py-6 max-h-[calc(100vh-200px)] overflow-y-auto">
              <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5">
                  {/* Nom complet - Full width on mobile, half on desktop */}
                  ...
                  <div className="md:col-span-2">
                    ...
                    <select
                      ...
                      className={`w-full px-3 sm:px-4 py-2 sm:py-2.5 text-sm sm:text-base border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all bg-white ${
                        isLoadingProperties ? 'bg-gray-100 cursor-not-allowed opacity-60' : ''
                      }`}
                    >
                      <option value="">
                    ...
                    {propertiesError && (
                      <p className="text-xs sm:text-sm text-red-600 mt-1.5">{propertiesError}</p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setNewTenant(prev => ({ ...prev, checkInDate: e.target.value }))}
                      className="w-full px-3 sm:px-4 py-2 sm:py-2.5 text-sm sm:text-base border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setNewTenant(prev => ({ ...prev, checkOutDate: e.target.value }))}
                      className="w-full px-3 sm:px-4 py-2 sm:py-2.5 text-sm sm:text-base border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all"
                    />
                  </div>
                ...
                {/* Action Buttons */}
                <div className="flex flex-col sm:flex-row justify-end gap-3 sm:gap-4 pt-4 sm:pt-5 mt-4 sm:mt-5 border-t border-gray-200">
                  <button
                    type="button"
                    onClick={() => setIsModalOpen(false)}
                  ...
                  <button
                    type="submit"
                    className="w-full sm:w-auto px-5 sm:px-6 py-2.5 sm:py-3 text-sm sm:text-base font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-all duration-200 shadow-sm hover:shadow-md"
                  >
      ...
      {isEditModalOpen && editingTenant && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-3 sm:p-4 md:p-6 overflow-y-auto">
          <div className="bg-white rounded-lg w-full max-w-2xl mx-auto my-auto shadow-xl">
            ...
            <div className="sticky top-0 bg-white z-10 px-4 sm:px-6 pt-4 sm:pt-6 pb-3 border-b border-gray-200 rounded-t-lg">
              <div className="flex justify-between items-center">
                <h2 className="text-lg sm:text-xl md:text-2xl font-semibold text-gray-800">Modifier le locataire</h2>
                <button
                  onClick={() => {
                    setIsEditModalOpen(false);
                    setEditingTenant(null);
